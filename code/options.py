...
def read_options():
    ...
    parser.add_argument("--train_entity_embeddings",
                        default=0, type=int, nargs="+")
    parser.add_argument("--train_relation_embeddings",
                        default=1, type=int, nargs="+")
    parser.add_argument("--total_iterations",
                        default=2000, type=int, nargs="+")
    ...
    parser.add_argument("--rounds_sub_training",
                        default=0, type=int, nargs="+")
    ...
    parser.add_argument("--learning_rate_judge",
                        default=5e-4, type=float, nargs="+")
    parser.add_argument("--learning_rate_agents",
                        default=5e-4, type=float, nargs="+")
    ...
    parsed['is_use_fixed_false_facts'] = (
        parsed['is_use_fixed_false_facts'] == 1)

    if parsed['pretrained_embeddings_path'] != '0':
        parsed['pretrained_embeddings_action'] = parsed['pretrained_embeddings_path'] + \
            '/' + "relations.p"
        parsed['pretrained_embeddings_entity'] = parsed['pretrained_embeddings_path'] + \
            '/' + "entities.p"
        parsed['pretrained_ent_dict'] = parsed['pretrained_embeddings_path'] + \
            '/' + "entities_dict.p"
        parsed['pretrained_rel_dict'] = parsed['pretrained_embeddings_path'] + \
            '/' + "relations_dict.p"
    else:
        parsed['pretrained_embeddings_action'] = '0'
        parsed['pretrained_embeddings_entity'] = '0'

    parsed['custom_baseline'] = list(
        map(lambda custom_baseline: custom_baseline == 1, parsed['custom_baseline']))
    # print and return
