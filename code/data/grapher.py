...
class RelationEntityGrapher:
    ...
    def __init__(self, data_input_dir, mode, entity_vocab, relation_vocab, mid_to_name, max_num_actions):
        ...
        if mid_to_name is None:
            ...
            self.rev_mid_to_name = dict([(v, k)
                                        for k, v in self.mid_to_name.items()])
        self.store = defaultdict(list)
        self.array_store = np.ones(
            (len(entity_vocab), max_num_actions, 2), dtype=np.dtype('int32'))
        self.array_store[:, :, 0] *= self.ePAD
        self.array_store[:, :, 1] *= self.rPAD
        self.rev_relation_vocab = dict(
            [(v, k) for k, v in relation_vocab.items()])
        ...
        for i in range(current_entities.shape[0]):
            ...
            if self.mode == 'train':
                ...
                if labels[i]:
                    ...
                    correct_obj = random.sample(
                        all_correct_answers[i // rollouts], 1)[0]

            if current_entities[i] == start_entities[i]:
                relations = ret[i, :, 1]
                entities = ret[i, :, 0]
                mask = np.logical_and(
                    relations == query_relations[i], entities == end_entities[i])
                ret[i, :, 0][mask] = self.ePAD
                ret[i, :, 1][mask] = self.rPAD

                mask_correct_answer = np.logical_and(
                    relations == correct_pred, entities == correct_obj)
