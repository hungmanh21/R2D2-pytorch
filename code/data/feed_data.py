...
class RelationEntityBatcher():
    ...
    def __init__(self, input_dir, batch_size, entity_vocab, relation_vocab, is_use_fixed_false_facts, num_false_facts=0,
        ...
        # TODO: Test if biased training for the agents changes something.
        self.rounds_sub_training = rounds_sub_training
        ...
        if self.mode == 'train':
            with open(self.input_dir+'/train.txt') as raw_input_file:
                ...
                for line in csv_file:
                    ...
                    # If no information, assume fact is correct.
                ...
                # Only true facts are considered
                    ...
                    for line in csv_file:
                        ...
                        if e1 in self.entity_vocab and e2 in self.entity_vocab:
                            ...
                            r = self.relation_vocab[r]
                            e2 = self.entity_vocab[e2]
                            # Make sure the data set and the mode are consistent.
                            assert len(line) == 3
                            is_correct = 1
                            self.store.append([e1, r, e2, is_correct])
                            self.store_relation_subjects[r].add(e1)
                            self.store_relation_object_subjects[(
                                r, e2)].add(e1)
    ...
    def generate_negative_object(self, subject, relation, used_set):
        ...
        try:
            ...
            diff_set = self.store_relation_objects[relation].difference(
                self.store_all_correct[(subject, relation)])
            rand_obj = random.sample(diff_set, 1)[0]
    ...
    def generate_negative_subject(self, relation, obj, used_set):
        ...
        try:
            ...
            diff_set = self.store_relation_subjects[relation].difference(
                self.store_relation_subjects[(relation, obj)])
            rand_sub = random.sample(diff_set, 1)[0]
    ...
    def generate_all_false_but_plausible_triples(self, subject, relation):
        ...
        set_negatives = self.store_relation_objects[relation].difference(
            self.store_all_correct[(subject, relation)])
        set_negative_facts = map(
            lambda object: [subject, relation, object, 0], set_negatives)
        ...
        for row in batch:
            ...
            try:
                for i in range(self.num_false_facts):
                    false_sub = self.generate_negative_subject(
                        relation, obj, used_sub)
    ...
    def generate_false_triples_biased(self, batch):
        ...
        for row in batch:
            ...
            try:
                for i in range(self.num_false_facts):
                    false_obj = self.generate_negative_object(
                        subject, relation, used_obj)
    ...
    def yield_next_batch_train(self):
        ...
        while True:
            batch_idx = np.random.randint(
                0, self.store.shape[0], size=self.batch_size)
            ...
            assert e1.shape[0] == e2.shape[0] == r.shape[0] == is_correct.shape[0] == len(
                all_e2s)
            iter_counter += 1
    ...
    def yield_next_batch_test(self):
        ...
        while True:
            ...
            if self.is_use_fixed_false_facts:
                ...
                list_negative_facts = list(map(lambda obj: [
                                           subject, predicate, obj, 0], self.store_false[(subject, predicate, correct_obj)]))
                ...
                subject = true_batch[0]
                predicate = true_batch[1]
                list_negative_facts = self.generate_all_false_but_plausible_triples(
                    subject, predicate)
                batch_list = [true_batch]
                batch_list += list_negative_facts
                batch = np.array(batch_list)
